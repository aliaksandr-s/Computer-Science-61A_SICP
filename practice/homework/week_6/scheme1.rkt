...
(define (eval-1 exp)
  (cond ((constant? exp) exp)
        ...
        ((and-exp? exp) 
          (cond [(null? (cdr exp)) #t] 
                [(eq? (eval-1 (cadr exp)) #f) #f]
                [else (if (null? (cddr exp)) 
                          (eval-1 (cadr exp)) 
                          (eval-1 (cons (car exp) (cddr exp))))]))
        ((map-exp? exp) (map-1 (cadr exp) (eval-1 (caddr exp))))
        ((lambda-exp? exp) exp)
        ((let-exp? exp) (eval-1 (substitute (caddr exp) 
                                            (map car (cadr exp)) 
                                            (map cadr (cadr exp)) 
                                            '())))
...
(define (map-1 proc lst) 
  (if (null? lst)
      '()
      (cons (apply-1 proc (list (car lst))) (map-1 proc (cdr lst)))))
